// Copyright 2024 INRAE, French National Research Institute for Agriculture,
...
// local
#include "romea_implement_teleop/implement_teleop.hpp"
...
ImplementTeleop::ImplementTeleop(const rclcpp::NodeOptions& options)
    : node_(std::make_shared<rclcpp::Node>("implement_teleop", options)),
      joy_(nullptr),
      cmd_pub_(nullptr) {
  try {
    declare_parameters_();
    ...
    cmd_pub_ = node_->create_publisher<std_msgs::msg::Float64MultiArray>(
        "lift_arm_controller/commands", 10);
...
void ImplementTeleop::init_joystick_() {
  ...
  auto callback = std::bind(&ImplementTeleop::joystick_callback_, this,
                            std::placeholders::_1);
...
ImplementTeleop::get_node_base_interface() const {
...
void ImplementTeleop::joystick_callback_(const Joystick& joy) {
  if (joy.getButtonValue("down_implement") ||
      joy.getAxeValue("up_down_implement") < -0.5) {
