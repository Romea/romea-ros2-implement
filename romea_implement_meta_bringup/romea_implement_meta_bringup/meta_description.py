...
import romea_implement_description

import yaml


class ImplementMetaDescription(SensorMetaDescription):
    def __init__(self, meta_description_file_path, robot_name=None):
        super().__init__("implement", meta_description_file_path, robot_name)

    # get gemoetry?


def load_meta_description(meta_description_file_path, robot_name=None):
    return ImplementMetaDescription(meta_description_file_path, robot_name)


def get_specifications(meta_description):
    return None
    # return romea_implement_description.get_specifications(meta_description.get_configuration())


def get_complete_configuration(meta_description):
    return romea_implement_description.get_complete_configuration(
...
def generate_yaml_configuration_file_str(meta_description, extended):
    ...
    # return romea_implement_description.generate_configuration_file_str(configuration, extended)


def generate_yaml_launch_file_str(meta_description):
    launch_file = meta_description.get_launch_file()
    pkg = "$(find-pkg-share romea_implement_meta_bringup)"
    ...
    return LaunchFileGenerator("implement").generate(
...
def generate_xml_urdf_description_str(mode, meta_description):
    ...
    return romea_implement_description.generate_urdf_description_str(
